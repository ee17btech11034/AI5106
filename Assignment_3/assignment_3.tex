...
\begin{align}
    \vec{x}^T \vec{x} -2 \vec{C}^T \vec{x} + c = 0 
\end{align}   
Circle is passing through points $\vec{P}, \vec{Q}, \vec{R}$. So, these points will satisfy the circle equation. We can create a determinant  :
\begin{align}
    \begin{vmatrix} x^2 + y^2 & x & y & 1\\ x_{1}^2 + y_{1}^2 & x_{1} & y_{1} & 1\\  x_{2}^2 + y_{2}^2 & x_{2} & y_{2} & 1 \\  x_{3}^2 + y_{3}^2 & x_{3} & y_{3} & 1 \end{vmatrix}
\end{align}
$M_{ij}$ is the minor $i^_{th}$ row and $j^_{th}$ column of above determinant. $\vec{C}$ is the centre of circle:\\
\begin{align}
    \vec{C} = \frac{1}{2M_{11}}\begin{pmatrix}  M_{12} \\ -M_{13} \end{pmatrix} \\
    c = \frac{-M_{14}}{M_{11}}
\end{align}
\section{Solution}
So, the centre of circle is :
\begin{align}
   \vec{C} = \frac{1}{2*8} \begin{pmatrix}  176 \\ -32 \end{pmatrix} = \begin{pmatrix}  11 \\ 2 \end{pmatrix}  \\
    c = \frac{-1*(-200)}{8} = 25 
\end{align}
